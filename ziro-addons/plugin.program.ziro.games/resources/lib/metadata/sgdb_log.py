...
LOG_TAG = "[Ziro Games SGDB]"
LOG_FILE = userdata_dir() / "sgdb.log"
...
def _write_file(line: str) -> None:
    try:
        ...
        xbmc.log(f"{LOG_TAG} could not write sgdb.log: {exc}", xbmc.LOGWARNING)


def _emit_chunks(level: int, prefix: str, text: str) -> None:
    if not text:
        xbmc.log(f"{prefix}", level)
        return
...
def log_http_error(status: int, url: str, body: str) -> str:
    ...
    summary = f"SteamGridDB HTTP {status}"
    if detail:
        summary = f"{summary}: {detail[:300]}"
    summary = f"{summary}\nFull URL and body are in sgdb.log"
    return summary
